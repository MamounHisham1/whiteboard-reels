...
```
If missing in all three, install from the official registry or skip it — rendering here uses a **local `remotion-project/`** (your own Primitives + theme + fonts), NOT the cloud `belt` CLI. The local project is what produces high-fidelity output. See `references/remotion-render-note.md`.
...
- `ELEVENLABS_API_KEY` — **required for Brian voice**. Get it: https://elevenlabs.io (free tier = 10k chars/mo). Falls back to Kokoro if unset, but Kokoro must be installed locally (see setup.md).
- `ELEVEN_VOICE` (optional) — ElevenLabs voice id, default `nPczCjzI2devNBz1zQrb` (Brian). Override if your account can't access that voice.
- `KOKORO_PYTHON` (optional) — path to a python with `kokoro` installed. No default; set it if you use the Kokoro fallback.
- `KOKORO_ONNX_MODEL` / `KOKORO_ONNX_VOICES` (optional) — paths to the kokoro-onnx model + voices files. No default.
...
0. **Check env** — `python3 scripts/check_env.py`. If `OPENCODE_KEY` or `ELEVENLABS_API_KEY` is missing, **stop and surface the gap** — point at `references/setup.md` and the `export` commands. Do NOT proceed to scaffold until required vars are set.
...
3. **Generate TTS** — `scripts/tts.py <slug>` → per-scene audio + `timing.json` + `full_narration.mp3`. Tries ElevenLabs Brian first; on quota/HTTP error, auto-falls back to Kokoro `am_michael` and prints `VOICE_USED=am_michael`. If neither ElevenLabs nor the `kokoro` package is available but `kokoro_onnx` is, run `scripts/tts_kokoro_onnx.py <slug>` instead. **When ElevenLabs quota is exhausted for a whole batch, keep the ENTIRE batch on `am_michael`** so that batch stays internally consistent — don't mix voices within one run.
4. **Write the Remotion scenes** — original diagrams for THIS topic. Copy font-loading + dependency set from the bundled assets. See `references/design-language.md` (exact palette, fonts, animation mechanics) and `references/build-pipeline.md`.
...
6. theme-match (palette/fonts/black-canvas match the design language)
...
**Fix loop:** until whole-video says clean OR cap (default 8 rounds) is hit. Each round: read defects → fix the exact elements named → save a new version (`video_vN.mp4`) → re-verify. **Never overwrite a version** — every render is preserved for review.

## Batch production

To build N videos: dispatch N sub-agents (one per topic), each running steps 1-7 independently. Report progress clearly so the operator can interrupt if something is off. **Do not** silently fall back to serial bash when sub-agents were requested.

## Operating rules

Read `references/operating-rules.md` once. Short version: work immediately (no ceremony), send whole videos to mimo (never frames), fix the exact element named in a defect report (not a neighbor), kill+retry hung mimo calls fast, and preserve every render version.
...
- `references/setup.md` — where to get the required API keys, how to set them, Kokoro install.
- `references/remotion-render-note.md` — why we use a local project, not the cloud belt CLI.
- `references/operating-rules.md` — operating rules for running this pipeline well.
...
- `scripts/tts_kokoro_onnx.py` — local Kokoro via `kokoro_onnx` (int8 ONNX); alternative fallback path.
...
- `scripts/check_env.py` — verify required env vars + tools are set before building.
